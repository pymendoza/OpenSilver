...
namespace Microsoft.Expression.Drawing.Core
{
	...
	internal static class PathFigureHelper
	{
		...
		internal static void FlattenFigure(PathFigure figure, IList<Point> points, double tolerance, bool removeRepeat)
		{
			...
			foreach (PathSegmentData item in figure.AllSegments())
			{
				item.PathSegment.FlattenSegment(list, item.StartPoint, tolerance);
			}
			if (figure.IsClosed)
			{
				list.Add(figure.StartPoint);
			...
			{
				double value = GeometryHelper.SquaredDistance(points.Last(), list[i]);
		...
		internal static bool SyncEllipseFigure(PathFigure figure, Rect bounds, SweepDirection sweepDirection, bool isFilled = true)
		{
			...
			if (size.Width > size.Height)
			{
				...
				reference = new Point(bounds.Left, point.Y);
				ref Point reference2 = ref array[1];
				reference2 = new Point(bounds.Right, point.Y);
			}
			else
			{
				ref Point reference3 = ref array[0];
				reference3 = new Point(point.X, bounds.Top);
				ref Point reference4 = ref array[1];
				reference4 = new Point(point.X, bounds.Bottom);
			}
			...
			flag |= GeometryHelper.EnsureSegmentType(out var result, figure.Segments, 0, () => new ArcSegment());
			flag |= result.SetIfDifferent(ArcSegment.PointProperty, array[1]);
			flag |= result.SetIfDifferent(ArcSegment.SizeProperty, size);
			flag |= result.SetIfDifferent(ArcSegment.IsLargeArcProperty, false);
			flag |= result.SetIfDifferent(ArcSegment.SweepDirectionProperty, sweepDirection);
			flag |= GeometryHelper.EnsureSegmentType(out result, figure.Segments, 1, () => new ArcSegment());
			flag |= result.SetIfDifferent(ArcSegment.PointProperty, array[0]);
			flag |= result.SetIfDifferent(ArcSegment.SizeProperty, size);
			flag |= result.SetIfDifferent(ArcSegment.IsLargeArcProperty, false);
			return flag | result.SetIfDifferent(ArcSegment.SweepDirectionProperty, sweepDirection);
		}
	}
}
#endif