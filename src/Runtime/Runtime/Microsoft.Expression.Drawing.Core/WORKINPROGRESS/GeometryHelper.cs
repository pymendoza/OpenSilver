...
namespace Microsoft.Expression.Drawing.Core
{
	...
	public static class GeometryHelper
	{
		...
		{
			if (size.Width > 0.0 && size.Height > 0.0 && !double.IsInfinity(size.Width))
			{
		...
		{
			return new Thickness(0.0 - thickness.Left, 0.0 - thickness.Top, 0.0 - thickness.Right, 0.0 - thickness.Bottom);
		}
		...
		{
			return Inflate(rect, new Thickness(offsetX, offsetY, offsetX, offsetY));
		}

		internal static Rect Inflate(Rect rect, Size size)
		{
			return Inflate(rect, new Thickness(size.Width, size.Height, size.Width, size.Height));
		}
		...
		{
			return Math.Atan2(point.Y - 0.5, point.X - 0.5) * 180.0 / Math.PI + 90.0;
		}
		...
		internal static Point SafeTransform(GeneralTransform transform, Point point)
		{
			...
			if (transform != null && transform.TryTransform(point, out outPoint))
			{
				return outPoint;
			}
			return point;
		...
		{
			return new Vector(0.0 - vector.Y, vector.X);
		}
		...
		internal static bool GeometryEquals(Geometry firstGeometry, Geometry secondGeometry)
		{
			...
			if (geometryGroup != null && geometryGroup2 != null)
			{
				return GeometryGroupEquals(geometryGroup, geometryGroup2);
			}
			return false;
		}
		...
		private static bool PathSegmentEquals(PathSegment firstSegment, PathSegment secondSegment)
		{
			...
			if (polyQuadraticBezierSegment != null && polyQuadraticBezierSegment2 != null)
			{
				return PolyQuadraticBezierSegmentEquals(polyQuadraticBezierSegment, polyQuadraticBezierSegment2);
			}
			return false;
		}
